...
impl<'a> Parser<'a> {
    ...
    pub fn with_capacity(capacity: uint) -> Result<Parser<'a>, ()> {
        ...
        try!(rxwork.recv_opt());
        Ok(Parser { stack: Vec::new(), capacity: capacity, tx: txmain, rx: rxwork })
    }
    ...
    pub fn parse<'b, 'c>(&'c mut self,
        ...
        let Parser { ref mut stack , capacity, ref tx, ref rx } = *guard;
